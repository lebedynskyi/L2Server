...
fun writeDebug(tag: String, msg: Any) {
    println("${timestamp()} $ANSI_RESET D: [$tag] $ANSI_RESET$msg")
}

fun writeInfo(tag: String, msg: Any) {
    println("${timestamp()} $ANSI_BLUE I: [$tag] $ANSI_RESET$msg")
}

fun writeError(tag: String, msg: Any, e: Throwable) {
    System.err.println("${timestamp()} $ANSI_RED E: [$tag] $ANSI_RESET$msg")
    e.printStackTrace()
}
